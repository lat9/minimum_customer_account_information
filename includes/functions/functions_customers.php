...
/*-bof-a-minacct*/
  ...
  $email_address = zen_output_string_protected($address['email_address']);
/*-eof-a-minacct*/
    if ($html) {
// HTML Mode
      $HR = '<hr />';
...
/*-bof-d-minacct
  ...
  -eof-d-minacct*/
/*-bof-a-minacct*/
    $address_out = $firstname . $lastname . $CR . $email_address . $CR . $telephone;
/*-eof-a-minacct*/
    return $address_out;
  }
...
/*    $address_query = "select entry_firstname as firstname, entry_lastname as lastname,
    ...
    $address_query = "select customers_firstname as firstname, customers_lastname as lastname,
                             customers_telephone as telephone, customers_email_address as email_address
...
//      $db->Execute("DELETE from " . TABLE_CUSTOMERS_BASKET . " WHERE customers_id= " . $customer_id);  lat9: deleted, function performed by the cart's reset(true) function
//      $db->Execute("DELETE from " . TABLE_CUSTOMERS_BASKET_ATTRIBUTES . " WHERE customers_id= " . $customer_id);  lat9: deleted, function performed by the cart's reset(true) function
      $_SESSION['cart']->reset('all');  /*lat9-changed: Properly delete any uploaded files associated with the cancelled transaction */
